...
public class UserService : IUserService
{
    ...
    public User GetUser(string token)
    {
        ...
        const string commandString = "select * from scores.user, scores.credentials where user.uusername = credentials.username and credentials.token = @token";
    ...
    public bool CreateUser(string firstName, string lastName, string username, string email, int phoneNumber, string pass, string pfp)
    {
        ...
        using var connection = new MySqlConnection(ConfigurationManager.ConnectionStrings["connectionString"].ConnectionString);
        const string credentialsString = "insert into scores.credentials (username, password, token) values (@username, @pass, @token)";
        const string usersString = "insert into scores.user (email, phone_number, first_name, last_name,pfp, uusername) values (@email, @phoneNumber, @firstName, @lastName, @pfp, @username)";
    ...
    {
        using var connection = new MySqlConnection(ConfigurationManager.ConnectionStrings["connectionString"].ConnectionString);
        const string commandString = "delete from scores.user where uusername = @username; delete from scores.credentials where username = @username";
        var command = new MySqlCommand(commandString, connection);
    ...
    public bool UpdateUser(string payloadToken, string? payloadFirstName, string? payloadLastName, string? payloadEmail, int? payloadPhoneNumber, string? payloadPfp)
    {
        ...
        using var connection = new MySqlConnection(ConfigurationManager.ConnectionStrings["connectionString"].ConnectionString);
        const string commandString = "update scores.user set email = @email , phone_number = @phonenumber , first_name = @firstname , last_name = @lastname , pfp = @pfp where uusername = @username";
        var command = new MySqlCommand(commandString, connection);
